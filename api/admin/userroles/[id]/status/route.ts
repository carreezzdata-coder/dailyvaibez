...
export async function PUT(
  ...
  try {
    ...
    if (!status || !['active', 'suspended'].includes(status)) {
      return handleError(400, 'Valid status is required (active or suspended)');
    }

    const backendUrl = getBackendUrl();
    const response = await fetch(`${backendUrl}/api/admin/userroles/${id}/status`, {
      method: 'PUT',
    ...
    return handleError(500, 'Failed to update user status');
  }
}