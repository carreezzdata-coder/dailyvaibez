...
export async function GET(request: NextRequest) {
  ...
  const requestId = `sidebar-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  
  try {
    const { searchParams } = new URL(request.url);
    const limit = searchParams.get('limit') || '12';
    
    console.log(`[${requestId}] Sidebar request: limit=${limit}`);

    const backendUrl = getBackendUrl();
    const endpoint = `${backendUrl}/api/sidebar?limit=${limit}`;
    ...
    console.log(`[${requestId}] Success: featured=${data.featured?.length || 0}, trending=${data.trending?.length || 0} in ${elapsed}ms`);
