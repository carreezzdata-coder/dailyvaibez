...
export async function GET(request: NextRequest) {
  ...
  const requestId = `pin-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  try {
    ...
    console.log(`[${requestId}] Pinned request: page=${page}, limit=${limit}, category=${categorySlug || 'all'}`);

    const backendUrl = getBackendUrl();
    const endpoint = categorySlug 
      ? `${backendUrl}/api/updates/pinned?page=${page}&limit=${limit}&category=${encodeURIComponent(categorySlug)}`
      : `${backendUrl}/api/updates/pinned?page=${page}&limit=${limit}`;
