...
export async function GET(request: NextRequest) {
  ...
  const requestId = `trend-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  try {
    ...
    console.log(`[${requestId}] Trending request: page=${page}, limit=${limit}, days=${days}`);

    const backendUrl = getBackendUrl();
    const endpoint = `${backendUrl}/api/updates/trending?page=${page}&limit=${limit}&days=${days}`;
