...
export async function fetchBreakingData(page: number = 1, limit: number = 50): Promise<BreakingData> {
  ...
  try {
    const response = await fastFetch(
      `${API_BASE_URL}/api/updates/breaking?page=${page}&limit=${limit}`,
      {
        method: 'GET',
    ...
    console.error('[BREAKING] Fetch error:', error);
    return {
      news: [],
      pagination: {
        current_page: 1,
