...
function getEmptySubCategoryContent(limit: number): SubCategoryContent {
  return {
    category: null,
    news: [],
...
export async function fetchSubCategoryContent(slug: string, page: number = 1, limit: number = 20): Promise<SubCategoryContent> {
  ...
  try {
    ...
    if (!response) {
      return getEmptySubCategoryContent(limit);
    }
