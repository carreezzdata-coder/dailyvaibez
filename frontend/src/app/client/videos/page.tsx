...
async function fetchVideosContent(
  page: number = 1,
  limit: number = 12,
  platform: string = 'all'
...
async function fetchVideoPlatforms(): Promise<VideoPlatformsResponse> {
  try {
...
export default async function VideosPage() {
  const videosData = await fetchVideosContent();
  const platformsData = await fetchVideoPlatforms();

  return (
    <div>
      <h1>Videos</h1>
      <p>Total Videos: {videosData.total_videos}</p>
      <p>Platforms: {platformsData.platforms.length}</p>
    </div>
  );
}