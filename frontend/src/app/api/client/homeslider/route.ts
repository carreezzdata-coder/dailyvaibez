...
export async function GET(request: NextRequest) {
  ...
  const requestId = `slider-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  
  try {
    console.log(`[${requestId}] Slider request started`);

    const backendUrl = getBackendUrl();
    const endpoint = `${backendUrl}/api/client/homeslider?_t=${Date.now()}`;
    ...
    if (!Array.isArray(data.slides)) {
      console.warn(`[${requestId}] slides is not an array:`, typeof data.slides);
      data.slides = [];
    }

    console.log(`[${requestId}] Success: ${data.slides.length} slides in ${elapsed}ms`);
    if (data.slides.length > 0) {
      console.log(`[${requestId}] First slide:`, data.slides[0].title);
    }
