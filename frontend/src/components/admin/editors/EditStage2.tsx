...
import ImageUpload from '../creators/ImageUpload';
...
const EditStage2: React.FC<EditStage2Props> = ({ 
  initialData, 
  onSubmit, 
  onBack,
  maxImages
  ...
  const [images, setImages] = useState<ImageFile[]>(() => {
    const existing = (initialData.existing_images || []).map(img => ({
      ...img,
      isUploading: false,
      hasWatermark: img.hasWatermark ?? false
    }));
    const newImages = (initialData.images || []).map(img => ({
      ...img,
      isUploading: false,
      hasWatermark: img.hasWatermark ?? false
    }));
  ...
  const handleSubmit = () => {
    ...
    const newImages = images.filter(img => !img.isExisting).map(img => ({
      ...img,
      isUploading: false,
      hasWatermark: img.hasWatermark ?? false
    }));
    ...
    <div className="admin-content">
      <div className="page-header" style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>
          ✏️ Edit Article - Stage 2
        </h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem' }}>
          Update your article content and images
        </p>
      </div>
...
- [TRANSCRIPT]...[/TRANSCRIPT] for interview transcripts"
      ...
      <div className="pagination">
        ...
        <button type="button" onClick={handleSubmit} className="new-post-btn">
