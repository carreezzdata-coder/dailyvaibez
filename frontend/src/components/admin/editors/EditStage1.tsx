...
const EditStage1: React.FC<EditStage1Props> = ({ 
  initialData, 
  onNext, 
  categoryGroups,
  allCategories
  ...
  useEffect(() => {
    if (categoryGroups.length > 0 && formData.category_ids.length > 0 && allCategories.length > 0) {
      const selectedGroups = new Set<string>();
      formData.category_ids.forEach(catId => {
        const cat = allCategories.find(c => c.category_id === catId);
        if (cat?.group) {
          selectedGroups.add(cat.group);
        }
      });
      setSelectedMainCategories(Array.from(selectedGroups));
    }
  }, [categoryGroups, allCategories, formData.category_ids]);
