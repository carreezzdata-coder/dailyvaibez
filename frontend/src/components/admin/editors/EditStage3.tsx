...
const EditStage3: React.FC<EditStage3Props> = ({ 
  initialData, 
  onSubmit, 
  onBack, 
  maxSocialLinks, 
  ...
  const [formData, setFormData] = useState<Stage3Data>(initialData);
  ...
  const publishButtonText = canPublish ? 'Update & Publish' : 'Submit for Approval';
    ...
    <div className="retrieve-posts">
      <div className="retrieve-header">
        <div className="header-left">
          <h1>Edit Article - Step 3: SEO & Social</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', margin: '0.5rem 0 0 0' }}>
            Update tags, metadata, and social media links
          </p>
        </div>
      </div>

      {requiresApproval && (
        <div className="quote-alert" style={{ background: 'rgba(255, 193, 7, 0.1)', borderColor: 'rgba(255, 193, 7, 0.3)', color: '#f59e0b', marginBottom: '1.5rem' }}>
          <span style={{ fontSize: '1.25rem', marginRight: '0.5rem' }}>⚠️</span>
          <span>
            <strong>Approval Required:</strong> Your changes will be submitted for review before updating the published post.
          </span>
        </div>
      )}

      <div className="posts-table">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 450px', gap: '2rem', marginBottom: '2rem' }}>
          <div>
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600, fontSize: '0.9rem' }}>Tags</label>
              <input
                type="text"
                value={formData.tags}
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600, fontSize: '0.9rem' }}>Meta Description (SEO)</label>
              <textarea
              ...
              <small style={{ display: 'block', marginTop: '0.5rem', color: 'var(--text-muted)', fontSize: '0.8rem' }}>
                {formData.meta_description.length} characters (recommended: 150-160)
              </small>
            </div>

            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600, fontSize: '0.9rem' }}>SEO Keywords</label>
              <input
                type="text"
          ...
          <div>
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-primary)', borderRadius: '12px', padding: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                <label style={{ margin: 0, fontSize: '1rem', fontWeight: 600 }}>🔗 Social Links</label>
                {formData.social_media_links.length < maxSocialLinks && (
                  <button
                    type="button"
        ...
        <div className="pagination" style={{ borderTop: '2px solid var(--border-primary)', paddingTop: '1.5rem' }}>
          <button 
            type="button" 
            onClick={onBack} 
            disabled={isSubmitting} 
