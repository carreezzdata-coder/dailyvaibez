...
export const useFeatured = (initialNews: NewsItem[] = []): UseFeaturedReturn => {
  ...
  const fetchFeatured = useCallback(async (): Promise<NewsItem[]> => {
    try {
      const response = await fetch(`/api/client/featured?limit=50&_t=${Date.now()}`, {
        method: 'GET',
        credentials: 'include',
        headers: {
      ...
      if (!result.success) {
        throw new Error(result.message || 'Failed to fetch featured news');
      }
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const featuredNews = await fetchFeatured();
      if (mountedRef.current) {
        setNews(featuredNews);
      }
    } catch (err) {
      if (mountedRef.current) {
        setError(err instanceof Error ? err.message : 'Failed to refresh featured news');
      }
    } finally {
      if (mountedRef.current) {
        setIsLoading(false);
