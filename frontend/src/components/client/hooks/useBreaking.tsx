...
export const useBreaking = (initialNews: NewsItem[] = []): UseBreakingReturn => {
  ...
  const fetchBreaking = useCallback(async (): Promise<NewsItem[]> => {
    try {
      const response = await fetch(`/api/client/breaking?limit=50&_t=${Date.now()}`, {
        method: 'GET',
        credentials: 'include',
        headers: {
      ...
      if (!result.success) {
        throw new Error(result.message || 'Failed to fetch breaking news');
      }
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const breakingNews = await fetchBreaking();
      if (mountedRef.current) {
        setNews(breakingNews);
      }
    } catch (err) {
      if (mountedRef.current) {
        setError(err instanceof Error ? err.message : 'Failed to refresh breaking news');
      }
    } finally {
      if (mountedRef.current) {
        setIsLoading(false);
